...
export class PayPalService extends BasePlatformService {
  protected readonly SANDBOX_API_URL = 'https://api.sandbox.paypal.com/v1';
  protected readonly PRODUCTION_API_URL = 'https://api.paypal.com/v1';
  ...
  private validateConfig(): void {
    if (!this.config.settings?.apiKey) {
      throw new Error('API Key é obrigatória para o PayPalService');
    }
    if (!this.config.settings?.secretKey) {
      throw new Error('Secret Key é obrigatória para o PayPalService');
    }
  }
  ...
  protected mapStatus(status: string): TransactionStatus {
    switch (status.toLowerCase()) {
      ...
      default:
        logger.warn(`Unknown status received from PayPal: ${status}`);
