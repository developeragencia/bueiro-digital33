...
export class PagTrustService extends BasePlatformService {
  protected readonly SANDBOX_API_URL = 'https://sandbox.pagtrust.com/api/v1';
  protected readonly PRODUCTION_API_URL = 'https://api.pagtrust.com/v1';
  ...
  getHeaders(): Record<string, string> {
    return {
      ...
      'X-PagTrust-Access-Token': this.config.settings?.apiKey || ''
    };
  }

  private mapPagTrustStatus(status: string): TransactionStatus {
  ...
  async processPayment(
    ...
    try {
      ...
      return {
        ...
        amount,
        currency,
        status: this.mapPagTrustStatus(response.data.status),
  ...
  async getTransactions(startDate?: Date, endDate?: Date): Promise<Transaction[]> {
    try {
      ...
      return response.data.transactions.map((transaction: any) => ({
        ...
        status: this.mapPagTrustStatus(transaction.status),
