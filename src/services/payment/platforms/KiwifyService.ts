...
export class KiwifyService extends BasePlatformService {
  protected readonly SANDBOX_API_URL = 'https://api.sandbox.kiwify.com/v1';
  protected readonly PRODUCTION_API_URL = 'https://api.kiwify.com/v1';
  ...
  private validateConfig(): void {
    if (!this.config.settings?.apiKey) {
      throw new Error('API Key é obrigatória para o KiwifyService');
    }
    if (!this.config.settings?.secretKey) {
      throw new Error('Secret Key é obrigatória para o KiwifyService');
    }
  }
  ...
  protected mapStatus(status: string): TransactionStatus {
    switch (status.toLowerCase()) {
      ...
      default:
        logger.warn(`Unknown status received from Kiwify: ${status}`);
