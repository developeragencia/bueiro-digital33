...
export class TictoService extends PaymentPlatformService {
  ...
  constructor(apiKey: string, secretKey: string, sandbox: boolean = true) {
    ...
    this.baseUrl = sandbox
      ? 'https://api.sandbox.ticto.com.br'
      : 'https://api.ticto.com.br';
    this.headers = {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${apiKey}`,
      'X-Account-Id': secretKey,
    };
  }

  async fetchOrders(): Promise<Transaction[]> {
    try {
      const response = await fetch(`${this.baseUrl}/v2/sales`, {
      ...
      console.error('Erro ao buscar vendas do Ticto:', error);
      throw error;
    }
  }
  ...
  private mapOrderToTransaction(sale: any): Transaction {
    return {
      ...
      metadata: {
        ...
        tracking: {
          utmSource: sale.tracking?.utm_source,
          utmMedium: sale.tracking?.utm_medium,
          utmCampaign: sale.tracking?.utm_campaign,
          utmTerm: sale.tracking?.utm_term,
          utmContent: sale.tracking?.utm_content,
          fbclid: sale.tracking?.fbclid,
          gclid: sale.tracking?.gclid,
        },
      },
    };
  }
  ...
  async syncTransactions(): Promise<void> {
    try {
      ...
      console.error('Erro ao sincronizar transações do Ticto:', error);
      throw error;
    }
  }
  ...
  async createWebhook(url: string): Promise<void> {
    try {
      ...
      console.error('Erro ao criar webhook no Ticto:', error);
      throw error;
    }
  }
  ...
  async handleWebhook(payload: any): Promise<void> {
    try {
      ...
      console.error('Erro ao processar webhook do Ticto:', error);
      throw error;
    }
  }
} 